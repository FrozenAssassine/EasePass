...
public static class DatabaseFormatHelper
{
        ...
        SecureString password)
    {
        ...
        //    MainDatabaseLoader.Save(source, password, default, file.database.Settings, file.database.Items);
        return file;
    }
    ...
    public static async Task<DatabaseValidationResult> Load(IDatabaseSource source, SecureString password, bool showWrongPasswordError)
    {
        ...
        if (preloadedDB.versionTag != DatabaseVersionTag.Undefined)
        {
            ...
            return await MainDatabaseLoader.Load(source, password, showWrongPasswordError, preloadedDB.data);
